...
import { getCustomFields, addCustomField, updateCustomField, deleteCustomField, type CustomField } from "~/models/settings";

type LoaderData = {
  scrutineerFields: CustomField[];
};

const defaultScrutineerFields: CustomField[] = [
  { id: 'default-1', section: 'scrutineer', field_name: 'Voted', field_type: 'boolean' },
];

export const loader: LoaderFunction = async () => {
  const customFields = await getCustomFields("scrutineer");
  return json<LoaderData>({ scrutineerFields: [...defaultScrutineerFields, ...customFields] });
};
...
export default function ScrutineerFields() {
  const { scrutineerFields } = useLoaderData<LoaderData>();
  ...
  return (
    <div>
      <h2 className="text-2xl mb-4">Scrutineer Fields</h2>
      {actionData?.error && (
        ...
        {scrutineerFields.map((field) => (
          <div key={field.id} className="p-2 border rounded flex items-center">
            <input
              ...
              disabled={typeof field.id === 'string' && field.id.startsWith('default')}
            />
            <select
      ...
      <div className="mt-4 p-2 border rounded">
        ...
        <Form method="post" onSubmit={handleAddCustomField} className="space-y-2">
          <input
            type="text"
            value={newFieldName}
